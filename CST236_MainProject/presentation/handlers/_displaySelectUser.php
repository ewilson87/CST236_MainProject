					...
					<div class="card-body text-left bg-info">
						...
						<div class="input-group mb-3">
							...
							<input type="text" class="form-control" name="email" value="<?php echo $user[0]['email'] ?>" 
							     aria-label="email" aria-describedby="email-addon" disabled>
						</div>
						<hr>
						<div class="input-group mb-3">
							<div class="input-group-prepend">
						...
						<div class="input-group mb-1">
							...
							<input type="text" class="form-control" name="accessLevel" value="<?php echo $user[0]['accessLevel'] ?>" aria-label="accessLevel"
								aria-describedby="accessLevel-addon" disabled>
							<div class="input-group-append">
								<span class="input-group-text">Note: 0 = User, 9 = Full Admin</span>
							</div>
						</div>
						<?php else: ?>
						<input type="hidden" id="accessLevel" name="accessLevel" value="<?php echo $user[0]['accessLevel'] ?>" disabled>
						<?php endif; ?>
				...
				<div class="card-body text-left bg-info">
					<h5 class="card-title">Account Edit Controls</h5>
						<form action="userAdminHandler.php" method="post">
							...
							<input type="hidden" id="ID" name="ID" value="<?php echo $user[0]['ID'] ?>">
							<select class="custom-select" id="userEditGroupSelect" name="userEditGroupSelect">
